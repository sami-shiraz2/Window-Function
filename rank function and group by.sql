...
select sale_cust_id, SUM(sale_total) AS total,
Rank() over (order by SUM(sale_total) desc) AS rnk,
dense_Rank() over (order by SUM(sale_total) desc) AS d_rnk
from sales__
